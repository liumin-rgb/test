const common = {
    ...
    getYearandMonth(str) { //"2020-08-25"->"20/08"
        return (new Date(str).getFullYear().toString().substring(new Date(str).getFullYear().toString().length, 2)) + "/" + ((new Date(str).getMonth() + 1).toString().length > 1 ? (new Date(str).getMonth() + 1).toString() : "0" + (new Date(str).getMonth() + 1).toString())
    },
    getPoptime(arr) { //获取最新更新时间
        arr.sort((a, b) => {
            return new Date(b.popTime) - new Date(a.popTime)
        })
        return (new Date(arr[0].popTime).getFullYear().toString().substring(new Date(arr[0].popTime).getFullYear().toString().length, 2)) + "/" + ((new Date(arr[0].popTime).getMonth() + 1).toString().length > 1 ? (new Date(arr[0].popTime).getMonth() + 1).toString() : "0" + (new Date(arr[0].popTime).getMonth() + 1).toString())
    },
    /**
     * 去除空格
     */
