<template>
  ...
  <div class="list-search">
   <div>
     ...
     <span class="floatR">
      <span class="backButton" @click="queryType=queryType=='low'?'high':'low'"><i class=""></i><span class="weight600">{{queryType=='low'?'普通搜索':'高级搜索'}}</span></span>
   ...
   <div>
     <span v-show="queryType=='high'"><span class="label">工作年限：</span><input class="pc-input" style="width:.6rem"/>--<input class="pc-input" style="width:.6rem"/></span>
...
<script>
  ...
  export default {
    name: 'staffInfo',
    components:{Pagination,ImportStaff,InsureImport,CategoryManage},
    data() {
      return{
        visible1:false,
        visible2:false,
    ...
    methods:{
      ...
      addStaff(){
        this.$router.push({path:'editStaff',query:{flag:1}});
      },
      openModel2(){
        this.visible2=true;
      },
